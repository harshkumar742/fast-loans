...
@router.post("/loan_applications/", response_model=models.LoanApplication)
async def create_loan_application(application: models.LoanApplication, db: Session = Depends(get_db)):
    db_application = crud.create_loan_application(db, application)
    produce_loan_application(db_application)
    return db_application

@router.get("/loan_applications/{application_id}/", response_model=models.LoanApplication)
...
@router.put("/loan_applications/{application_id}/", response_model=models.LoanApplication)
def update_loan_application(application_id: int, application: models.LoanApplication, db: Session = Depends(get_db)):
    db_application = crud.update_loan_application(db, application_id, application)
    if db_application is None:
        raise HTTPException(status_code=404, detail="Application not found")
    return db_application

@router.delete("/loan_applications/{application_id}/", response_model=dict)
