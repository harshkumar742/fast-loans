...
def test_create_loan_application():
    response = client.post("/loan_applications/", json=application_data.dict())
    assert response.status_code == 200
    assert "id" in response.json()
    assert response.json()["applicant_name"] == "John Doe"

def test_get_loan_application():
    response_post = client.post("/loan_applications/", json=application_data.dict())
    application_id = response_post.json()["id"]

    response_get = client.get(f"/loan_applications/{application_id}/")
...
def test_update_loan_application():
    ...
    response_put = client.put(f"/loan_applications/{application_id}/", json=updated_data)
...
def test_delete_loan_application():
    ...
    response_delete = client.delete(f"/loan_applications/{application_id}/")
